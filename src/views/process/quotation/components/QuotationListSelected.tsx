    ...
    const handleConfirmDelete = () => {
        const newQuotationList = quotationList.filter((quotation) => {
            return !selectedQuotation.some(
                (selected) => selected.id === quotation.id,
            )
        })
        setSelectAllQuotation([])
        mutate(
            {
                list: newQuotationList,
    ...
    return (
        <>
            ...
            >
                ...
                <p>Send message to the following quotation</p>
                <Avatar.Group
                    chained
                ...
                >
                    {selectedQuotation.map((quotation) => (
                        <Tooltip key={quotation.id} title={quotation.name}>
                            <Avatar size={30} src={quotation.img} alt="" />
                        </Tooltip>
                    ))}
                </Avatar.Group>
                <div className="my-4">
